...
import { startGameCountdown } from '../entities/escape/countdown';
import { spawnEnemies } from '../entities/enemy';
import { spawnPlayer } from '../entities/player';
import { displayPortalCountdown, spawnPortal } from '../entities/escape/portal';
...
export const initGameScene = ({
  ...
  function main() {
    k.setGravity(1600);
    const player = spawnPlayer({ k, level, speed, playerSprite, endless });
    generateWorld(k, endless);
    startGameCountdown();
    spawnGameObjects(k);

    const currentLevelLabel = !endless
      ? k.add([k.text(`Level: ${level}`), k.pos(24, 24)])
      : null;
    k.onUpdate(() => {
